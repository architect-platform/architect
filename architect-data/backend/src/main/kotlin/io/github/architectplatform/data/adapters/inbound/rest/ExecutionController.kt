...
import io.github.architectplatform.data.adapters.inbound.rest.dto.ExecutionEventResponse
import io.github.architectplatform.data.adapters.inbound.rest.dto.ExecutionResponse
import io.github.architectplatform.data.adapters.inbound.rest.dto.ReportEventRequest
import io.github.architectplatform.data.adapters.inbound.rest.dto.ReportExecutionRequest
import io.github.architectplatform.data.application.domain.ExecutionStatus
import io.github.architectplatform.data.application.ports.inbound.TrackExecutionEventUseCase
...
class ExecutionController(
    ...
    @Get("/{id}")
    fun getExecution(@PathVariable id: String): ExecutionResponse? {
        return trackExecutionUseCase.getExecution(id)?.let { ExecutionResponse.fromDomain(it) }
    }
    ...
    @Get("/recent")
    fun getRecentExecutions(@QueryValue(defaultValue = "50") limit: Int): List<ExecutionResponse> {
        return trackExecutionUseCase.getRecentExecutions(limit)
            .map { ExecutionResponse.fromDomain(it) }
    }
    ...
    fun getEventsByExecution(@PathVariable executionId: String): List<ExecutionEventResponse> {
        return trackExecutionEventUseCase.getEventsByExecution(executionId)
